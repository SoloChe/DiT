...
def main(args):
    ...
    _, age_map, _ = get_age(args.age_path, prefix=args.prefix, round_age=True) # age:index
    ...
    if args.resume_checkpoint is not None:
        ...
        model, ema, opt, resume_steps, scale_factor = load_from_checkpoint(model, ema, opt, args.resume_checkpoint, load_sf=True)
    else:
        resume_steps = 0
        scale_factor = None
    ...
    logger.info(f"VAE Parameters: {sum(p.numel() for p in vae.parameters())/1024/1024:.2f}M")
    
    prefix = None if args.prefix == 'all' else args.prefix
    if args.dim == 3: 
        # dataset = BrainDataset_3D(args.data_path, args.age_path, mode="train")
        dataset = BrainDataset_3D(args.data_path, args.age_path, prefix=prefix, mode="train", round_age=True, crop=True)
    else:
        dataset = BrainDataset_2D(args.data_path, args.age_path, prefix=prefix, mode="train", round_age=True, crop=True)
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    ...
                    _, _ = sample_from_noise(model.module, diffusion, device, args, name=train_steps, 
                                             decoder=vae.decode_stage_2_outputs, scale_factor=scale_factor)
                    model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-size", type=int, choices=[28, 32, 128, 224, 256, 512], default=256)
    parser.add_argument("--in-channels", type=int, default=3)
    parser.add_argument("--pos-embed-dim", type=int, default=3) # posional embedding: 1d:1 2d:2 3d:3 or learned:4
